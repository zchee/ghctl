...
fmt: tools/bin/goimportz tools/bin/gofumpt  ## Run goimportz and gofumpt.
	$(call target)
	${TOOLS_BIN}/goimportz -local=${PKG},$(subst /jsonrpc2,,$(PKG)) -w $(shell find . -iname "*.go" -not -path "./vendor/**")
	${TOOLS_BIN}/gofumpt -s -extra -w $(shell find . -iname "*.go" -not -path "./vendor/**")
	...
	@cd tools; \
	  for t in ${TOOLS}; do \
			if [ -z '$*' ] || [ $$(basename $$t) = '$*' ]; then \
				echo "Install $$t ..."; \
				GOBIN=${TOOLS_BIN} CGO_ENABLED=0 go install -v -mod=mod $(strip ${GO_FLAGS}) "$${t}"; \
			fi \
	  done
